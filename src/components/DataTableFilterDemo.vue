...
export default {
  ...
  watch: {
    agentValue(val) {
      this.agentModel = val;
      console.log("Test>>>", this.agentModel);
    },
  },
  setup() {
    onMounted(() => {
      customerService.value.getCustomersLarge().then((data) => {
        ...
        testData.value = data.filter((d) => d.country.name === agentModel);
        console.log("Data>>Test", testData.value);
        testData.value.map((res, id) =>
          resData.value.push({
            name: res.representative.name,
            image: res.representative.image,
          })
        );
        console.log("Res>>>", resData.value);
        customers1.value.forEach(
          (customer) => (customer.date = new Date(customer.date))
        );
      });
      console.log("Value", agentModel.value);
    ...
    const representatives = ref(resData.value);
    const statuses = ref([
      "unqualified",
      "qualified",
      "new",
